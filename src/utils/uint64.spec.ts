...
describe('uint64', () => {
    it('converts 1', () => {
        ...
        expect(unint64.uint64ToStringLE(buf)).toMatchInlineSnapshot(`"1"`);
    });
    it('converts 256', () => {
        const buf = new Uint8Array([0, 1, 0, 0, 0, 0, 0, 0]);
        expect(unint64.uint64ToStringLE(buf)).toMatchInlineSnapshot(`"256"`);
    });
    it('converts a large number', () => {
        const buf = new Uint8Array([0, 0, 0, 0, 1, 0, 0, 0]);
        expect(unint64.uint64ToStringLE(buf)).toMatchInlineSnapshot(
            `"4294967296"`,
        );
    });
