...
function parseFractionalPrice(numberFormatting, s, formatFlags, decimals) {
	...
	if (formatFlags.ModernFractions) {
		...
		let separator = getModernFractionsSeparator(numberFormatting);
		let denominator = 1 << decimals;

		let pipIndex = s.indexOf(separator);
		if (pipIndex !== -1) {
			let integerPart = s.substring(0, pipIndex).trim();
			...
			if (pipIndex + 1 < s.length) {
				let pipPart = numberFormatting.parse(s.substring(pipIndex + 1).trim());
		...
		let fracIndex = findFractionalPart(s);

		if (fracIndex !== -1 && fracIndex < s.length) {
			let integerPart = s.substring(0, fracIndex).trim();
			result = (integerPart.length > 0 ? numberFormatting.parse(integerPart) : 0.0);

			let fractionalPart = s.substring(fracIndex).trim();
			let isVulgarFraction = false;

			if (fractionalPart.length === 1) {
				let vulgarIndex = fractionChars.indexOf(fractionalPart);
				if (vulgarIndex >= 0) {
			...
			if (!isVulgarFraction) {
				let divIndex = indexOfArray(fractionalPart, divisionChars);
				if (divIndex !== -1 && divIndex < fractionalPart.length) {
					let numeratorPart = fractionalPart.substring(0, divIndex).trim();
					let denominatorPart = fractionalPart.substring(divIndex + 1).trim();

					let numeratorParsed = parseFloat(numeratorPart);
					let denominatorParsed = parseFloat(denominatorPart);
					if (numeratorParsed < denominatorParsed) {
						let frac = numeratorParsed / denominatorParsed;
						if (result >= 0) {
							result += frac;
						} else {
							result -= frac;
