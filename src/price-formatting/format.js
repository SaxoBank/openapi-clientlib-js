...
function formatPricePartsFraction(parts, numberFormatting, value, decimals, formatFlags, numeratorDecimals) {
	...
	if (formatFlags.NoRounding) {
		...
		let resolution = 1 << numeratorDecimals;
...
function formatPricePartsDecimals(parts, numberFormatting, value, decimals, formatFlags) {
	...
	if (formatFlags.Percentage) {
		...
		let extra = decimals + 1;
		let fullPrice = formatNumber(value, extra, numberFormatting);
 ...
 * @param {string|Object.<string, boolean>} [formatFlags="Normal"] - Indicates if the price also include half-pips (decimal pips), and which format should be used.
