...
StreamingOrphanFinder.prototype.update = function() {
	...
	var now = Date.now(),
		oldNextUpdateIn = this.nextUpdateTime - now, // old next Update In
		newNextUpdateIn = Infinity, // new oldNextUpdateIn
	...
	for(let i = 0, subscription; subscription = this.subscriptions[i]; i++) {
		let timeTillOrphaned = subscription.timeTillOrphaned(now);
	...
	if (this.minCheckTime > now) {
		let startDelayEndsIn = this.minCheckTime - now;
