...
function onTokenChanged(token, expiry) {
	...
	if (elapse <= 0) {
		...
		if (elapse < 0) {
			log.warn(LOG_AREA, "token has changed, but expiry is less than the token refresh margin.", { expiry: expiry, tokenRefreshMarginMs: this.tokenRefreshMarginMs });
			elapse = 0;
		}
...
function getToken(url) {
    ...
    this.transport.fetch(this.tokenRefreshMethod, url, { headers: headers, cache: false, credentials: this.tokenRefreshCredentials })
...
/**
 * This class builds on top of {@link saxo.openapi.TransportCore} and adds authentication management. You need only construct one or the other,
 * they automatically wrap each other. All of the options from the {@link saxo.openapi.TransportCore} constructor are valid here as well.
