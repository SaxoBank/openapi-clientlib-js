...
describe("openapi TransportAuth", () => {
    ...
    describe("auth store", () => {
        ...
        it("gets and sets", () => {
            var options = {token: "Bearer TOKEN", expiry: relativeDate(60), tokenRefreshUrl: 'refresh'};
            ...
            var newAuth = {token: "Bearer TOK2", expiry: relativeDate(60)};
        ...
        it("gets and sets adding Bearer", () => {
            var options = {token: "TOKEN", expiry: relativeDate(60), tokenRefreshUrl: 'refresh'};
            ...
            var newAuth = {token: "TOK2", expiry: relativeDate(60)};
            transportAuth.auth.set(newAuth.token, newAuth.expiry);
    ...
    describe("auth events", function() {
        it("fires an event when about to refresh", function() {
            var options = {token: "TOKEN", expiry: relativeDate(60), tokenRefreshUrl: 'http://refresh'};
            transportAuth = new TransportAuth('localhost/openapi', options);

            var tokenRefreshSpy = jasmine.createSpy("tokenRefresh listener");
        ...
        it("fires an event when receiving a new token", function(done) {
            ...
            var tokenReceivedSpy = jasmine.createSpy("tokenReceived listener");
            var tokenRefreshFailSpy = jasmine.createSpy("tokenRefreshFail listener");
    ...
    describe("refreshing", () => {
        ...
        it("allows no arguments and defaults to expired", function() {
            var options = {tokenRefreshUrl: 'http://refresh'};
            transportAuth = new TransportAuth('localhost/openapi', options);
        ...
        it("refreshes immediately if the token is out of date", (done) => {
            var initialOptions = {token: "TOKEN", expiry: relativeDate(-1), tokenRefreshUrl: "http://refresh"};
        ...
        it("includes the headers passed in as options", () => {
            var headersOption = { foo: "bar"};
            var expectedHeaders = { foo: "bar", "Content-Type": "JSON" };

            var initialOptions = {token: "TOKEN", expiry: relativeDate(-1), tokenRefreshUrl: "http://refresh", tokenRefreshHeaders: headersOption};
            transportAuth = new TransportAuth('localhost/openapi', initialOptions);
        ...
        it("includes the headers passed in as options and does not override content-type", () => {
            var expectedHeaders = { foo: "bar", "Content-Type": "JSON" };
            var headersOption = expectedHeaders;
        ...
        it("refreshes when the token becomes out of date", (done) => {
            var initialOptions = {token: "TOKEN", expiry: relativeDate(10), tokenRefreshUrl: 'http://refresh'};
            transportAuth = new TransportAuth('localhost/openapi', initialOptions);
        ...
        it("retries if the auth refresh fails", (done) => {
            var initialOptions = {token: "TOKEN", expiry: relativeDate(10), tokenRefreshUrl: 'http://refresh', retryDelayMs: 599, maxRetryCount: 1};
            transportAuth = new TransportAuth('localhost/openapi', initialOptions);
        ...
        it("does nothing if the token is in date", () => {
            var initialOptions = {token: "TOKEN", expiry: relativeDate(60), tokenRefreshUrl: 'http://refresh'};
            transportAuth = new TransportAuth('localhost/openapi', initialOptions);
