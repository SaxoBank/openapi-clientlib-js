...
describe('openapi Streaming', () => {
    ...
    describe('options', () => {
        it('has defaults', () => {
            new Streaming(transport, 'testUrl', authProvider);
            expect(mockConnection.start.calls.count()).toEqual(1);
            expect(mockConnection.start.calls.argsFor(0)[0]).toEqual({ waitForPageLoad: false, transport: ['webSockets', 'webSockets', 'longPolling'] });
        });

        it('can override waitForPageLoad', () => {
            new Streaming(transport, 'testUrl', authProvider, { waitForPageLoad: true });
            expect(mockConnection.start.calls.count()).toEqual(1);
            expect(mockConnection.start.calls.argsFor(0)[0]).toEqual({ waitForPageLoad: true, transport: ['webSockets', 'webSockets', 'longPolling'] });
        });
