...
describe('log', () => {
    ...
    it('works when it has subscribers', () => {
        ...
        const debugSpy = jasmine.createSpy(log.DEBUG);
        log.on(log.DEBUG, debugSpy, expectedContextObject);
        const infoSpy = jasmine.createSpy(log.INFO);
        log.on(log.INFO, infoSpy, expectedContextObject);
        const warnSpy = jasmine.createSpy(log.WARN);
        log.on(log.WARN, warnSpy, expectedContextObject);
        const errorSpy = jasmine.createSpy(log.ERROR);
